...
public class SleepRadioSearchRepository {
    ...
    public SleepRadioSearchRepository() {
        this(new RadioBrowserService(new SimpleHttpClient()));
    }

    public SleepRadioSearchRepository(@NonNull RadioBrowserService radioBrowserService) {
        this.radioBrowserService = radioBrowserService;
    }

    @NonNull
    public List<SleepRadioStation> search(@NonNull String query) throws IOException {
        String trimmedQuery = query.trim();
        if (trimmedQuery.isEmpty()) {
            return mapStations(radioBrowserService.searchStations(null, "China", SEARCH_LIMIT), "");
        }
        ...
        List<SleepRadioStation> results = new ArrayList<>(merged.values());
        Collections.sort(results, Comparator.comparingInt(station -> -scoreStation(station, trimmedQuery)));
        if (results.size() > SEARCH_LIMIT) {
            return new ArrayList<>(results.subList(0, SEARCH_LIMIT));
        }
        return results;
    }

    public void registerClick(@NonNull SleepRadioStation station) {
        try {
    ...
    private void appendMatches(@NonNull Map<String, SleepRadioStation> merged,
                               @NonNull List<SleepRadioStation> stations) {
    ...
    @NonNull
    private List<SleepRadioStation> mapStations(@NonNull List<RadioBrowserStation> source,
    ...
    private int scoreStation(@NonNull SleepRadioStation station, @NonNull String query) {
